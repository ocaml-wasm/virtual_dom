...
let many_without_merge ?(here = Stdlib.Lexing.dummy_pos) attrs =
...
let id ?(here = Stdlib.Lexing.dummy_pos) s = create ~here "id" s
let name ?(here = Stdlib.Lexing.dummy_pos) s = create ~here "name" s
let href ?(here = Stdlib.Lexing.dummy_pos) r = create ~here "href" r
let rel ?(here = Stdlib.Lexing.dummy_pos) r = create ~here "rel" r
let label ?(here = Stdlib.Lexing.dummy_pos) r = create ~here "label" r
let target ?(here = Stdlib.Lexing.dummy_pos) s = create ~here "target" s
let checked = create ~here:[%here] "checked" ""
let checked_prop ?(here = Stdlib.Lexing.dummy_pos) b = bool_property ~here "checked" b
...
let placeholder ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "placeholder" x
let role ?(here = Stdlib.Lexing.dummy_pos) r = create ~here "role" r

let autofocus ?(here = Stdlib.Lexing.dummy_pos) = function
...
let title ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "title" x
let alt ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "alt" x
let src ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "src" x
let open_ = create ~here:[%here] "open" ""
let start ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "start" (Int.to_string x)
let min ?(here = Stdlib.Lexing.dummy_pos) x = create_float ~here "min" x
let max ?(here = Stdlib.Lexing.dummy_pos) x = create_float ~here "max" x
let min_date ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "min" (Date.to_string x)
let max_date ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "max" (Date.to_string x)

let min_date_time ?(here = Stdlib.Lexing.dummy_pos) x =
  create ~here "min" (Date.to_string x ^ "T00:00")
;;

let max_date_time ?(here = Stdlib.Lexing.dummy_pos) x =
  create ~here "max" (Date.to_string x ^ "T23:59")
;;

let colspan ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "colspan" (Int.to_string x)
let rowspan ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "rowspan" (Int.to_string x)
let rows ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "rows" (Int.to_string x)
let cols ?(here = Stdlib.Lexing.dummy_pos) x = create ~here "cols" (Int.to_string x)

let draggable ?(here = Stdlib.Lexing.dummy_pos) b =
  create ~here "draggable" (Bool.to_string b)
;;
...
module Multi = struct
  ...
  let map_style ?(here = Stdlib.Lexing.dummy_pos) t ~f =
    [ Many_only_merge_classes_and_styles
        { attrs = t; map_styles = f; map_classes = Fn.id; here }
    ]
  ;;

  let add_class ?(here = Stdlib.Lexing.dummy_pos) t c =
    [ Many_only_merge_classes_and_styles
  ...
  let merge_classes_and_styles ?(here = Stdlib.Lexing.dummy_pos) t =
    [ Many_only_merge_classes_and_styles
...
let create ?(here = Stdlib.Lexing.dummy_pos) name value = create ~here name value
...
let string_property ?(here = Stdlib.Lexing.dummy_pos) name value =
  string_property ~here name value
;;

let bool_property ?(here = Stdlib.Lexing.dummy_pos) name value =
  bool_property ~here name value
;;
